                ...
                for i=1:N
                    if i~=idx_lead
                        ...
                        for j=1:h
                            ...
                            if booth(X1new,X2new)<fitnessnew
                                x1new = X1new; x2new = X2new; fitnessnew = booth(X1new,X2new);
                                movefl = 1;
                            end
                        end
                        if movefl
                        ...
                        if booth(X(i,1),X(i,2))<lead
                            lead = booth(X(i,1),X(i,2));
                            idx_lead = i;
                            brkfl=0;
                            break
                        end
        ...
        elseif step==3
            ...
            while distfl
                ...
                for i=1:length(moveable_wolves)
                    ...
                    refreshdata
                    drawnow
                    if booth(X(moveable_wolves(i),1), X(moveable_wolves(i),2))<lead
                        lead = booth(X(moveable_wolves(i),1), X(moveable_wolves(i),2));
        ...
        elseif step==4
            ...
            for i=1:N
                if i~=idx_lead
                    ...
                    if booth(x1new, x2new)<booth(x1old, x2old)
                        X(i,1) = x1new; X(i,2) = x2new;
                        m = X(:,1); n = X(:,2);
                        refreshdata
                        drawnow
                        if booth(x1new, x2new)<lead
                            lead = booth(x1new, x2new);
                            idx_lead = i;
                        end
                    end
                end
